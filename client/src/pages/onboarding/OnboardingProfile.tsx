...
// rate. 1 = strictest filter (top-25 only), 4 = loosest (hides only
// open-admission), null = no filter.
const ACADEMIC_TIER_OPTIONS: { value: 1 | 2 | 3 | 4 | 5 | null; label: string; detail: string }[] = [
  { value: 1,    label: 'Most selective',    detail: 'Top-25. Stanford, Duke, the Ivies.' },
  { value: 2,    label: 'Highly selective',  detail: 'Top-50. UNC, UVA, Notre Dame, BC.' },
  { value: 3,    label: 'Selective',         detail: 'Top-100. Most flagship state schools.' },
  { value: 4,    label: 'Less selective',    detail: 'All four-year colleges except open-admission.' },
  { value: null, label: 'No academic filter', detail: "Don't filter by academics at all." },
]
            ...
            {step.key === 'goals' && (
              <>
                ...
                <p className="text-xs text-ink-3 leading-[1.6] -mt-1">
                  How picky are you on academics? We'll hide anything below this bar.
                </p>
                <div className="grid grid-cols-1 gap-2">
                  {ACADEMIC_TIER_OPTIONS.filter((opt) => opt.value !== null).map((opt) => (
                    <Chip
                      key={opt.value ?? 'any'}
                      active={(form.academic_minimum ?? null) === opt.value}
                      onClick={() => update('academic_minimum', opt.value)}
                    >
                      <span className="block text-left font-mono text-[12px] tracking-[0.10em] font-semibold">{opt.label}</span>
                      <span className="block text-left text-[11px] text-ink-3 mt-1.5 leading-[1.5]">{opt.detail}</span>
                    </Chip>
                  ))}
                </div>
                <button
                  type="button"
                  onClick={() => update('academic_minimum', null)}
                  className={`block w-full text-left text-xs leading-[1.5] mt-1 transition-colors ${
                    (form.academic_minimum ?? null) === null
                      ? 'text-gold'
                      : 'text-ink-3 hover:text-ink-1'
                  }`}
                >
                  {(form.academic_minimum ?? null) === null ? '✓ ' : ''}Skip this — show every school regardless of academics
                </button>
              </>
            )}

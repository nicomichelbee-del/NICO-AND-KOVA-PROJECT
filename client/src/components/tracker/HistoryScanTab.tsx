...
/** Pill shown when interest and genuineness diverge by 2+ points. */
function DivergencePill({ score, genuineness }: { score: number; genuineness: number }) {
  const diff = score - genuineness
  if (Math.abs(diff) < 2) return null
  if (diff >= 2) {
    // High interest, low genuineness — likely a recruiting blast
    return (
      <span className="px-2 py-0.5 rounded text-[10px] font-semibold border bg-[rgba(251,191,36,0.1)] border-[rgba(251,191,36,0.25)] text-[#fbbf24]">
        📢 Mass outreach
      </span>
    )
  }
  // High genuineness, low interest — warm but no forward asks yet
  return (
    <span className="px-2 py-0.5 rounded text-[10px] font-semibold border bg-[rgba(148,163,184,0.1)] border-[rgba(148,163,184,0.2)] text-[#94a3b8]">
      💬 Warm, no asks yet
    </span>
  )
}
...
function EmailCard({
  ...
  onCancelAdd: () => void
  onAdd: () => void
  onCoachNameChange: (v: string) => void
  onSchoolNameChange: (v: string) => void
  onDivisionChange: (v: Division) => void
}) {
  ...
  return (
    <div className={`rounded-xl border border-[rgba(255,255,255,0.07)] bg-[rgba(255,255,255,0.02)] border-l-4 ${rCfg.border} overflow-hidden`}>
      <div className="p-4">
        {/* ── Top row ── */}
        <div className="flex items-start justify-between gap-3 flex-wrap mb-2">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-0.5">
              ...
              <DivergencePill score={email.score} genuineness={email.genuineness} />
            </div>
        ...
        {/* ── Score bars — side-by-side so divergence is immediately visible ── */}
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div>
            <div className="flex justify-between mb-1">
              {/* INTEREST = forward momentum: asks, next steps, timelines */}
            ...
            <div className="text-[9px] text-[#334155] mt-0.5">asks · next steps · timeline</div>
          </div>
          <div>
            <div className="flex justify-between mb-1">
              {/* GENUINENESS = personal engagement: reply depth, personalization, tone */}
            ...
            <div className="text-[9px] text-[#334155] mt-0.5">replies · personalization · tone</div>
          </div>
        </div>
        ...
        {!tracked && !adding && (
          <div className="mt-3 pt-3 border-t border-[rgba(255,255,255,0.06)] flex justify-end">
            <Button size="sm" variant="outline" onClick={onStartAdd}>+ Add to Tracker</Button>
          </div>
        )}
      </div>
      ...
      {adding && (
        <div className="px-4 pb-4 flex items-end gap-3 flex-wrap border-t border-[rgba(255,255,255,0.07)] pt-4">
          <div className="flex-1 min-w-36">
            <Input label="School name" value={addSchoolName} onChange={(e) => onSchoolNameChange(e.target.value)} placeholder="University name" />
          </div>
          <div className="flex-1 min-w-36">
            <Input label="Coach name" value={addCoachName} onChange={(e) => onCoachNameChange(e.target.value)} placeholder="Coach name" />
          </div>
          <div>
            <label className="text-xs text-[#64748b] block mb-1.5">Division</label>
            <select value={addDivision} onChange={(e) => onDivisionChange(e.target.value as Division)}
          ...
          <Button size="sm" onClick={onAdd} disabled={addLoading || !addSchoolName}>
...
export function HistoryScanTab({ userId, gmailConnected, onContactAdded }: Props) {
  ...
  // Separate real recruiting emails from noise (mass camp blasts, newsletters)
  const { main: mainEmails, noise: noiseEmails } = useMemo(() => {
    const main: HistoryEmail[] = []
    const noise: HistoryEmail[] = []
  ...
  const stats = useMemo(() => ({
    total: mainEmails.length,
    hot:   mainEmails.filter((e) => e.rating === 'hot').length,
    warm:  mainEmails.filter((e) => e.rating === 'warm').length,
    cold:  mainEmails.filter((e) => e.rating === 'cold' || e.rating === 'not_interested').length,
    untracked: mainEmails.filter((e) => !e.isTracked && !addedIds.has(e.threadId)).length,
  }), [mainEmails, addedIds])
  ...
  const visible = useMemo(() => {
    ...
    return list
  }, [mainEmails, categoryFilter, trackFilter, ratingFilter, sortBy, addedIds])
  ...
  if (!gmailConnected) {
    return (
      <Card className="p-12 text-center">
        ...
        <p className="text-xs text-[#64748b]">Connect Gmail to scan 3 years of your inbox for every coach who ever contacted you.</p>
      </Card>
    )
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card className="p-6">
        <div className="flex items-start justify-between gap-4 flex-wrap mb-1">
          <div>
            <div className="font-bold text-[#f1f5f9] text-base mb-1">3-Year Email Intelligence Scan</div>
            <p className="text-xs text-[#64748b] max-w-lg">
              ...
              every real coach or ID camp email from the past 3 years. Filters out spam, platforms, and mass blasts automatically,
              then rates each email's interest and personal engagement with AI.
            </p>
          </div>
          <Button onClick={handleScan} disabled={loading}>
            {loading ? 'Scanning…' : scanned ? '↻ Re-scan' : '🔍 Scan 3 Years'}
          </Button>
        </div>
          ...
          {visible.length === 0 ? (
            <Card className="p-10 text-center">
              ...
              <div className="text-sm text-[#64748b]">
                {mainEmails.length === 0 ? 'No coach emails found in the past 3 years.' : 'No emails match the current filters.'}
              </div>
            </Card>
          ) : (
            <div className="flex flex-col gap-3">
              {visible.map((email) => (
                <EmailCard
                  key={email.threadId}
                  email={email}
                  isAdded={addedIds.has(email.threadId)}
                  adding={adding === email.threadId}
                  addCoachName={addCoachName}
                  addSchoolName={addSchoolName}
                  addDivision={addDivision}
                  addLoading={addLoading}
                  onStartAdd={() => { setAdding(email.threadId); setAddCoachName(''); setAddSchoolName('') }}
                  onCancelAdd={() => setAdding(null)}
                  onAdd={() => handleAdd(email)}
                  onCoachNameChange={setAddCoachName}
                  onSchoolNameChange={setAddSchoolName}
                  onDivisionChange={setAddDivision}
                />
              ))}
            </div>
          )}
          ...
          {noiseEmails.length > 0 && (
            <div>
              <button
                ...
                className="flex items-center gap-2 text-xs text-[#475569] hover:text-[#64748b] transition-colors mb-3">
                <span className="text-[10px] border border-[rgba(255,255,255,0.08)] rounded px-2 py-0.5 bg-[rgba(255,255,255,0.03)]">
                  {noiseExpanded ? '▾' : '▸'}
                </span>
                <span>Filtered out — {noiseEmails.length} mass blast{noiseEmails.length !== 1 ? 's' : ''} / newsletter{noiseEmails.length !== 1 ? 's' : ''}</span>
                <span className="text-[10px] text-[#334155]">(ID camp blasts, recruiting newsletters — not real recruiting contact)</span>
              </button>

              {noiseExpanded && (
                <div className="flex flex-col gap-2">
                  {noiseEmails.map((email) => (
                    <div key={email.threadId}
                      className="rounded-xl border border-[rgba(255,255,255,0.05)] bg-[rgba(255,255,255,0.01)] px-4 py-3 flex items-start gap-3 opacity-60">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-0.5">
                          <span className="text-xs text-[#64748b]">{email.senderName || email.senderEmail}</span>
                          {email.noiseReason && (
                            <span className="px-1.5 py-0.5 rounded text-[10px] border bg-[rgba(71,85,105,0.15)] border-[rgba(71,85,105,0.3)] text-[#475569]">
                              {email.noiseReason}
                            </span>
                          )}
                        </div>
                        <div className="text-[11px] text-[#334155] italic truncate">"{email.subject}"</div>
                      </div>
                      <span className="text-[10px] text-[#334155] flex-shrink-0">{formatDate(email.date)}</span>
                    </div>
                  ))}
                </div>
              )}
